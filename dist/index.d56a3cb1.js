...
async function fetchCountryInfo() {
    try {
        const response = await _axiosDefault.default.get(URI + ENDPOINT);
        ...
        response.data.map((country)=>{
            ...
            const itemName = document.createElement("li");
            function colorName() {
                if (country.region === "Oceania") return itemName.setAttribute("id", "oceania");
                if (country.region === "Africa") return itemName.setAttribute("id", "africa");
                if (country.region === "Europe") return itemName.setAttribute("id", "europe");
                if (country.region === "Americas") return itemName.setAttribute("id", "americas");
                if (country.region === "Asia") return itemName.setAttribute("id", "asia");
            }
        ...
        if (err.response.status === 404) errorMessage.textContent = "page not found | 404";
        //check welke error message van toepassing is
        if (err.response.status === 500) errorMessage.textContent = "Internal server status | 404";
    }
}
fetchCountryInfo();
...
async function fetchSpecificCountryInfo(inputFieldValue) {
    ...
    try {
        ...
        const response = await _axiosDefault.default.get("https://restcountries.com/v2/name/" + inputFieldValue);
        ...
        function currenciesCountry() {
            ...
            for(let i = 1; i < lengthCurrenciesArray; i++)itemCurrency.textContent += ` and with ${country.currencies[i].name}'s`;
        ...
        function languagesCountry() {
            ...
            for(let i = 1; i < lengthLanguageArray; i++)itemLanguage.textContent += ` and ${country.languages[i].name}`;
            return itemCurrency.textContent;
        }
        ...
        const errorMessage = document.getElementById("error-Message");
        //check welke error message van toepassing is
        if (err.response.status === 404) errorMessage.textContent = "NAAM LAND IS NIET JUIST INGEVOERD OF BESTAAT NIET";
        //check welke error message van toepassing is
        if (err.response.status === 500) errorMessage.textContent = "Internal server status | 404";
    }
